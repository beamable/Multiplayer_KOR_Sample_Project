...
using UnityAsync;
using UnityEngine;
using static Beamable.Samples.KOR.Data.GameProgressData;
         ...
         await AsyncUtility.AsyncSafe(async () =>
         {
            switch (_gameState)
            {
               ...
               case GameState.RoundEvaluated:
                  ...
                  GameProgressData.RoundResult currentRoundResult = _gameSceneManager.GameProgressData.CurrentRoundResult;

                  switch (currentRoundResult)
                  {
                     case GameProgressData.RoundResult.Tie:
                        ...
                        await SetGameState(GameState.RoundStarting);
                        return;
                     case GameProgressData.RoundResult.Winner:
